...
CustomPass::CustomPass(XRLib::Graphics::VkCore& core, XRLib::Scene& scene,
                       std::vector<std::unique_ptr<XRLib::Graphics::IGraphicsRenderpass>>* renderPasses, bool stereo)
    : VkStandardRB(core, scene, renderPasses, stereo) {
    if (!stereo) {
...
void CustomPass::Prepare() {
    ...
    auto descriptorSet =
        std::make_unique<XRLib::Graphics::DescriptorSet>(core, viewProjectBuffer, modelPositionBuffer, textures);
    descriptorSet->AllocatePushConstant(sizeof(uint32_t));
    descriptorSets.push_back(std::move(descriptorSet));

    auto graphicsRenderPass = std::make_unique<XRLib::Graphics::VkGraphicsRenderpass>(
        core, stereo, swapchain->GetSwapchainImages(), std::move(descriptorSets), vertexShaderPath, fragmentShaderPath);
...
void CustomPass::InitViewProjectBuffer() {
    ...
    XRLib::EventSystem::Callback<std::vector<glm::mat4>, std::vector<glm::mat4>> bufferCamUpdateCallback =
        [&](std::vector<glm::mat4> views, std::vector<glm::mat4> projs) {
            ...
            viewProjectBuffer->UpdateBuffer(sizeof(XRLib::Graphics::Primitives::ViewProjectionStereo),
                                            static_cast<void*>(&viewProjStereo));
        };
...
void CustomPass::InitModelPositionBuffer() {
    ...
    XRLib::EventSystem::Callback<> modelPositionBufferCallback = [&]() {
        ...
        modelPositionBuffer->UpdateBuffer(sizeof(glm::mat4) * modelPositions.size(),
                                          static_cast<void*>(modelPositions.data()));
    };

    XRLib::EventSystem::RegisterListener(XRLib::Events::XRLIB_EVENT_APPLICATION_PRE_RENDERING,
                                         modelPositionBufferCallback);
}
...
std::vector<std::shared_ptr<XRLib::Graphics::Image>> CustomPass::InitTexture() {
    ...
    if (textures.empty()) {
        ...
        textures.push_back(
            std::make_shared<XRLib::Graphics::Image>(core, textureData, 1, 1, 4, VK_FORMAT_R8G8B8A8_SRGB));
        return textures;
    }

    for (int i = 0; i < textures.size(); ++i) {
        textures[i] = std::make_shared<XRLib::Graphics::Image>(
